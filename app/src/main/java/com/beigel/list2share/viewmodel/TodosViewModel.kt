package com.beigel.list2share.viewmodel
...
class TodosViewModel(
        ...
        viewModelScope.launch {
            try {
                repository.updateTodo(
                    ...
                    assignedTo, reminderMinutes, recurrence, rotateAmong, quantity, price, link
                )
    ...
    fun addSubtask(todo: TodoItem, title: String) {
        if (title.isBlank()) return
        val newSubtask = Subtask(id = UUID.randomUUID().toString(), title = title.trim())
        viewModelScope.launch {
            try {
